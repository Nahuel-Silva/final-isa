...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'dateAdded' | 'dateModified'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      dateAdded: product.dateAdded?.format(DATE_FORMAT) ?? null,
      dateModified: product.dateModified?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      dateAdded: restProduct.dateAdded ? dayjs(restProduct.dateAdded) : undefined,
      dateModified: restProduct.dateModified ? dayjs(restProduct.dateModified) : undefined,
    };
  }
